...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    private float sightMoveSpeed; // 좌우 움직임 스피드
    [SerializeField]
    private float sightSensitivity; // 고개 움직임 속도
    [SerializeField]
    private float lookLimitX;
    [SerializeField]
    private float lookLimitY;
    ...
    private void NotCamUI()
    {
        ...
        if(currentAngleY >= lookLimitX) // 현재 카메라 오른쪽 방향 각도가 제한 영역 오른쪽 각도보다 크거나 같으면
            go_NotCamRight.SetActive(true);
        else if(currentAngleY <= -lookLimitX) // 현재 카메라 오른쪽 방향 각도가 제한 영역 왼쪽 각도보다 작거나 같으면
            go_NotCamLeft.SetActive(true);
        
        if(currentAngleX >= lookLimitY) // 현재 카메라 아래 방향 각도가 제한 영역 아래 각도보다 크거나 같으면
            go_NotCamDown.SetActive(true);
        else if(currentAngleX <= -lookLimitY) // 현재 카메라 아래 방향 각도가 제한 영역 위 각도보다 작거나 같으면   
    ...
    private void KeyViewMoving()
    {
        ...
        {
            // 현재 카메라 오른쪽 방향 각도에 좌우 키 입력 값 * 고개 움직임 속도를 더함
            currentAngleY += sightSensitivity * Input.GetAxisRaw("Horizontal");
            // 현재 카메라 오른쪽 방향 각도를 제한 영역 왼쪽 각도와 오른쪽 각도 사이로 제한
            currentAngleY = Mathf.Clamp(currentAngleY , -lookLimitX , lookLimitX);
            // 카메라의 x축 위치에 좌우 키 입력 값 * 좌우 움직임 스피드를 더함
    ...
    {   // 크로스헤어의 x 포지션이 화면 오른쪽 끝 - 100 의 값보다 크거나 || 왼쪽 끝 + 100 보다 작으면
        if (tf_Crosshair.localPosition.x > (Screen.width / 2 - 100) || tf_Crosshair.localPosition.x < (-Screen.width / 2 + 100))
        {
            ...
            currentAngleY = Mathf.Clamp(currentAngleY , -lookLimitX , lookLimitX);
            // 왼쪽으로 움직이면 sightMovespeed 빼줌 오른쪽은 더해줌
